package com.wangxin.chinesepoker.Manager;

import com.wangxin.chinesepoker.Adapter.AnalyzerAdapter;
import com.wangxin.chinesepoker.Adapter.AptTarget;
...
public class CardsManager {
	...
	public static int compare(CardsHolder f, CardsHolder s) {
		// 当两种牌型相同时
		if (f.getCardsType().getIndex() == s.getCardsType().getIndex() ) {
			// 数量不同将无法比较
			if (f.getCardsLength() != s.getCardsLength())
				return -1;
			else {
				if (f.getCardsValue() > s.getCardsValue()) {
					return 1;
				}
				else {
					return 0;
		...
		// 在牌型不同的时候,火箭最大
		if (f.getCardsType().getIndex() == CardsType.huojian) {
			return 1;
		}
		if (s.getCardsType().getIndex() == CardsType.huojian) {
			return 0;
		}
		// 在牌型不同的时候,排除火箭的类型，炸弹最大
		if (f.getCardsType().getIndex() == CardsType.zhadan) {
			return 1;
		}
		if (s.getCardsType().getIndex() == CardsType.zhadan) {
			return 0;
		}
		// 无法比较的情况，默认为s大于f
		return -1;
	...
	public static int[] outCardByItsself(int cards[], Player last, Player next) {
		AptTarget analyze = new AnalyzerAdapter();
	...
	public static int[] findTheRightCard(CardsHolder card, int cards[], Player last, Player next) {
		...
		// 判断我该不该要牌
		if (Desk.boss != last.getPlayID() && Desk.boss != next.getPlayID()) {
			// 我是boss，就要要牌
			// 判断他的剩余牌数
			int pokeLength = Desk.players[card.getPlayID()].getCards().length;
		...
		if (Desk.boss == last.getPlayID()) {
			if (card.getPlayID() == last.getPlayID()) {
				int pokeLength = Desk.players[card.getPlayID()].getCards().length;
				int must = pokeLength * 100 / 17;
				if (pokeLength <= 2) {
					must = 100;
				...
				if (card.getPlayID() == next.getPlayID()) {
					if (c <= 3) {
						return findBigerCards(card, cards, 100);
					}
					return null;
		...
		if (Desk.boss == next.getPlayID()) {
			if (card.getPlayID() == last.getPlayID()) {
				if (card.getCardsValue() < 12) {
					...
					ana.setPokes(next.getCards());
					if (ana.remainCount() <= 1) {
						if (ana.lastCardTypeEq(card.getCardsType())
								&& (Desk.boss == next.getPlayID() || (Desk.boss != next.getPlayID() && Desk.boss != last.getPlayID()))) {
							return findBigerCards(card, cards, 100);
						}
					}
					else {
	...
	public static int[] findBigerCards(CardsHolder card, int cards[], int must) {
		try {
			// 获取card的信息，牌值，牌型
			int[] cardPokes = card.getCards();
			int cardValue = card.getCardsValue();
			int cardType = card.getCardsType().getIndex();
			int cardLength = cardPokes.length;
			// 使用AnalyzePoke来对牌进行分析
			AptTarget analyzer = new AnalyzerAdapter();
